...
```{r setup, message=FALSE, eval=FALSE, eval=FALSE}
library(eyetools)
library(gganimate)
library(tidyverse)
library(zoo)
...
#anim_save("figures/point_lag.gif", plot_animate, height = 810, width = 1440, duration = round(max(data_plot1$time)/1000),
#          end_pause = 5)
```
...
# To save an animation
anim_save("figures/raw_smooth_single.gif", plot_animate_smooth, height = 810, width = 1440, duration = round(max(data_both$time)/1000),
          end_pause = 5)
```
...
```{r, include=FALSE, eval = FALSE}
# To save an animation
anim_save("figures/raw_smooth_lag.gif", plot_animate3, height = 810, width = 1440, duration = round(max(data_both$time)/1000),
          end_pause = 5)
```

```{r, echo=FALSE, fig.cap='Image showing lagged timepoint difference between raw and smoothed data'}
knitr::include_graphics("figures/raw_smooth_lag.gif")
```
...
This is easily done by specifying NAs in the appropriate position in the `AOI_names` argument and using the `keep_out_of_AOI == FALSE` option.

```{r, eval=FALSE}

growth_partial <- data_smooth |> 
  filter(pNum == "118", trial == "1") |> 
  plot_AOI_growth(AOIs = HCL_AOIs, keep_out_of_AOI = FALSE, AOI_names = c("Predictive", "Non Predictive", NA)) +
  geom_point() + 
  transition_reveal(time)
