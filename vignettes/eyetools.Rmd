...
# matthew - what is the purpose of showing these library commands?
library(eyetools)
library(ggplot2) # this doesn't need to be loaded, right?
...
Let's load in this binocular data and explore the format. As we can see, the dataset is over 462,000 rows and 7 observations
...
To get a basic idea of the contents of the data, we can look at the select data from the first timepoint in six trials for participant number 118.

[this seems a slightly odd thing to look at in the data. Maybe just look at the first 20 timestamps from a suitable trial?]
...
[it seems that at this stage "pNum" is lost in the eye data. It's not clear how you then manage to the merge the data with the td data....]
...
plot_seq(data[[2]], trial_number = 57, AOIs = AOI_areas) # add AOIs
