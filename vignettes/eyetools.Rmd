...
# participant 118, trial 5  
# y coord
ggplot() +
  geom_line(data = data[data$pNum == 118 & data$trial == 5,],
            aes(x = time, y = y),
            colour = "red") +  
  geom_line(data = data_smooth[data_smooth$pNum == 118 & data_smooth$trial == 5,],              
            aes(x = time, y = y),             
            colour = "blue") +   
  theme_bw()
```
The plot above shows the difference between the raw and smoothed data. With the default smoothing setting, we can see that the smoothed data does not track the actual data as closely as it could. The lower the value of `span`, the closer the smoothed data represents the raw data. The difference is minimal, but a visual inspection suggests that a span of .02 is a good value for this data example. It is important that the fixations and saccades are matched closely to ensure data quality.
...
data <- merge(data, data_behavioural) # merges with the common variables pNum and trial
...
data_fixations_VTI <- fixation_VTI(data_nonNA,
             threshold = 100,
             min_dur = 150, # Minimum duration (in milliseconds) of period over which fixations are assessed
...
#take two trial for easier visualisation
data_subset <- data_118[data_118$trial %in% 2:3,]

#data_subset$pNum <- NULL
...
plot_spatial(fix_data = data_fixations_disp[data_fixations_disp$trial == 1,]) # a LOT of fixations, let's take a sample
