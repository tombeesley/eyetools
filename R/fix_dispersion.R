...
#' @keywords internal
#'
fix_dispersion_SLOW <- function(data, min_dur = 150, disp_tol = 100) {

  data <- split(data,data$trial) # create list from data by trial
  data_fix <- data %>% map(~trial_level_process_SLOW(., min_dur = min_dur, disp_tol = disp_tol))
...
trial_level_process_SLOW <- function(data, min_dur, disp_tol, round = 0) {
